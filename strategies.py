...
# Function that returns decoding function for each strategy
...
def beam_search_strategy(logits, num_beams=4):
    ...
    prob_mask = prob_mask / prob_mask.sum(dim=-1, keepdim=True) 
    prob_mask = torch.clamp(prob_mask, min=1e-12)  
    return prob_mask
